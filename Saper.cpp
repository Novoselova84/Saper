...
int field[100][100];//поле
// -1 - mine
// 0 - 8 cell
int field_p[100][100];//поле игрока
/*
* -2 closed
* -1 flag
* 0 - 8 opened
...
void print_field()//вывод поле игрока
{
	...
	for (int i = 0; i < N; ++i) {
		...
		if (i + 1 < 10) cout << ' ';
		for (int j = 0; j < M; ++j) {
			if (field_p[i][j] == -2)cout << "_  ";       //если -2 закрыто
			else if (field_p[i][j] == -1) cout << "f  "; // если -1 flag и от 0 - 8 opened	
...
void end_game(bool is_win = false)
{
	for (int i = 0; i < N; i++) {
		for (int j = 0; j < M; j++) {
			...
			if (field[i][j] == -1) field_p[i][j] = -3;//здесь стоит мина
		}
	}
	print_field();
...
int main()
{
	while (true) {
		...
		for (int i = 0; i < N; i++) {
			for (int j = 0; j < M; j++) {
				field[i][j] = 0;//обнул€ем поле
				field_p[i][j] = -2;//закрываем клетки
			}
		}

		for (int i = 0; i < K; ++i) {//генерируем поле минами
			while (true) {
				int x = rand() % N;
				int y = rand() % M;
				if (field[x][y] != -1) {
		...
		for (int i = 0; i < N; i++) {//устанавливаем р€дом с минами кол-во мин в €чейках
			for (int j = 0; j < M; j++) {
				if (field[i][j] != -1) {
					field[i][j] = 0;
		...
		while (true) {//ходы игрока
			print_field();
			cout << "Enter comand:";
			/*
				* \o x, y - ввод координат
				* \f x, y - ставим флаг
				* \n- играем заново
				* \q конец игры
			*/
